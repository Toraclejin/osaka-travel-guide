...
├── overview.html              구 북마크용 리다이렉트 스텁 → index.html (v0.16 폐기 → v0.24 부활, 지우지 말 것)
...
| 🔴 | **태풍 (8월 말 성수기)** | 양방향 배라 결항하면 대체 교통이 없다. **출발 2~3일 전 예보** + 팬스타 **결항·환불·재승선 정책** 확인. 특히 복귀일(8/26)이 걸리면 일정 전체가 흔들린다. 앱 `departure` warn 에 반영됨 (v0.22) |
