...
| ✕ | `overview.html` | **일행 공유용 구조도** — 구역·원정·테마·공백을 한 장으로 | 데이터 변경마다 |

> ⚠ `overview.html`은 **파생 산출물**이다. 장소·구역의 정본은 `index.html`의 `GUIDE_DATA`/`EXCURSIONS`이고, 여기에는 그것을 옮겨 적었다.
> **데이터를 고치면 이 파일도 같은 커밋에서 고친다.** 안 고치면 R-D1이 경고한 "정본 행세하는 두 번째 사본"이 된다.
> 앱이 아니므로 비번 게이트가 없다 — **공개 위치에 올릴 때 주의** (§ 6 권한 경계 아래 배포 주의 참조).
